...
double sumLogProb(std::vector<double>& logprobs)
{
    ...
    if (isinf(max)) { // the largest probability is 0 (log prob= -inf)
        ...
        double p = 0;
        for (i = 0; i < logprobs.size(); i++) {
            p += std::exp(logprobs[i] - max);
        }
        return max + std::log(p);
    }
}
...
{
    if (isinf(logprob1) and isinf(logprob2)) {
        return logprob1; // both prob1 and prob2 are 0, return log 0.
    }
    if (logprob1 > logprob2) {
        return logprob1 + std::log(1 + std::exp(logprob2 - logprob1));
    } else {
        return logprob2 + std::log(1 + std::exp(logprob1 - logprob2));
    }
}
